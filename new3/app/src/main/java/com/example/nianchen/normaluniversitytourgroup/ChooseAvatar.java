...
public class ChooseAvatar extends BaseActivity {
...
//        edit = (EditText)findViewById(R.id.EditDetal);
        ...
        textView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                String url = "http://123.207.228.232/blog/addimage";
                AsyncHttpClient client = new AsyncHttpClient();
                RequestParams rp = new RequestParams();
                rp.put("username",EMClient.getInstance().getCurrentUser().toString().trim());
                rp.put("url", getFileName(pathss));
                rp.put("gg",b);
...
//                edit = (EditText)findViewById(R.id.EditDetal);
                textView = (TextView)findViewById(R.id.tv);
                dataList.addLast(null);// 初始化第一个添加按钮数据
                adapter = new UploadImageAdapter(ChooseAvatar.this, dataList);
                uploadGridView.setAdapter(adapter);
                uploadGridView.setOnItemClickListener(mItemClick);
                uploadGridView.setOnItemLongClickListener(mItemLongClick);
//                edit.setText("");
...
//            data.get
                ...
                if (!TextUtils.isEmpty(uri.getAuthority())) {
                    ...
                    paths=pathImage;
                    b = paths.substring(paths.lastIndexOf(".") + 1, paths.length());
                    Log.e("格式",b);
//                    EMClient.getInstance().getCurrentUser().toString().trim();
                }
